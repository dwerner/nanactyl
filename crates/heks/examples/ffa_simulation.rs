...
fn batch_spawn_entities(world: &mut World, n: usize) {
    ...
    // We could instead call `world.spawn((pos, s, hp, dmg, kc))` for each
    // entity, but `spawn_batch` is faster.
}
...
fn system_fire_at_closest(world: &mut World) {
    ...
    {
        // Find closest:
        // Nested queries are O(n^2) and you usually want to avoid that by using some
        // sort of spatial index like a quadtree or more general BVH, which we
        // don't bother with here since it's out of scope for the example.
        let closest = world
