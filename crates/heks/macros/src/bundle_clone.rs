...
fn gen_dynamic_bundle_impl(
    ...
    quote! {
        unsafe impl #impl_generics ::heks::DynamicBundleClone for #ident #ty_generics #where_clause {
            #[allow(clippy::forget_copy)]
            unsafe fn put_with_clone(mut self, mut f: impl ::std::ops::FnMut(*mut u8, ::heks::TypeInfo, ::heks::DynamicClone)) {
                #(
                    f(
                        (&mut self.#field_members as *mut #tys).cast::<u8>(),
                        ::heks::TypeInfo::of::<#tys>(),
                        ::heks::DynamicClone::new::<#tys>()
                    );
