...
///
/// Bundles can be passed directly to `World::spawn` and `World::insert`, and
/// obtained from `World::remove`. Can be convenient when combined with other
/// derives like `serde::Deserialize`.
///
/// # Example
/// ```
/// # use heks::*;
...
///
/// Queries structs can be passed to the type parameter of `World::query`. They
/// must have exactly one lifetime parameter, and all of their fields must be
/// queries (e.g. references) using that lifetime.
///
/// # Example
/// ```
/// # use heks::*;
