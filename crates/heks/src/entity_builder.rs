...
/// Helper for incrementally constructing a bundle of components with dynamic
/// component types
///
...
impl EntityBuilder {
    ...
    ///
    /// If the bundle contains any component which matches the type of a
    /// component already in the `EntityBuilder`, the newly added component
    /// from the bundle will replace the old component and the old component
    /// will be dropped.
    ...
    /// [`EntityRef::get`](crate::EntityRef::get), even though it cannot be a
    /// unique reference because `EntityBuilder` does not perform dynamic
    /// borrow checking.
    pub fn get<'a, T: ComponentRefShared<'a>>(&'a self) -> Option<T> {
        self.inner.get::<T>()
    }

    /// Borrow a shared or unique reference `T` to some component type, if it
    /// exists
    ...
    ///
    /// The builder is cleared implicitly when an entity is built, so this
...
/// [`World::spawn`](crate::World::spawn) or
...
impl Drop for BuiltEntity<'_> {
    fn drop(&mut self) {
        // Ensures components aren't leaked if `store` was never called, and prepares
...
impl EntityBuilderClone {
    ...
    /// Add `component` to the entity.
    ///
    /// If the bundle already contains a component of type `T`, it will be
    /// dropped and replaced with the most recently added one.
    ...
    /// Convert into a value whose shared references are [`DynamicBundle`]s
    /// suitable for repeated spawning
    ...
    /// Takes a reference as its type parameter for consistency with
    /// [`EntityRef::get`](crate::EntityRef::get), even though it cannot be a
    /// unique reference because `EntityBuilderClone` does not perform
...
///
/// Built from, and convertible back into, [`EntityBuilderClone`].
/// `DynamicBundle` is implemented for *references to* this type, allowing it to
/// be e.g. spawned repeatedly.
#[derive(Clone)]
