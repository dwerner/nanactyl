...
impl<'a> EntityRef<'a> {
    ...
    /// For example, this can be combined with a `HashMap<TypeId, Box<dyn
...
impl<'a, T: ?Sized> RefMut<'a, T> {
    /// Transform the `RefMut<'_, T>` to point to a part of the borrowed data,
    /// e.g. a struct field.
    ///
