...
pub trait Query {
    ...
    ///
    /// This is usually the same type as the query itself, except with an
    ...
    ///
    /// # Safety
    /// - Must only be called after [`Fetch::borrow`] or with exclusive access
    ///   to the archetype
    /// - [`Fetch::release`] must not be called while `'a` is still live
    /// - Bounds-checking must be performed externally
    /// - Any resulting borrows must be legal (e.g. no &mut to something another
    ///   iterator might access)
    unsafe fn get<'a>(fetch: &Self::Fetch, n: usize) -> Self::Item<'a>;
}

/// Marker trait indicating whether a given [`Query`] will not produce unique
/// references
...
/// A query that matches all entities, yielding `bool`s indicating whether each
/// satisfies query `Q`
///
/// Does not borrow any components, making it faster and more
/// concurrency-friendly than `Option<Q>`.
///
/// # Example
/// ```
/// # use heks::*;
...
impl<'w, Q: Query> QueryBorrow<'w, Q> {
    ...
    ///
    /// Convenient when the values of the components in the other query are not
    /// of interest.
    ///
...
impl<'q, Q: Query> QueryIter<'q, Q> {
    /// # Safety
    ///
    /// `'q` must be sufficient to guarantee that `Q` cannot violate borrow
    /// safety, either with dynamic borrow checks or by representing
...
impl<'q, Q: Query> QueryMut<'q, Q> {
    ...
    /// Like `into_iter`, but returns child iterators of at most `batch_size`
    /// elements
    ///
    /// Useful for distributing work over a threadpool.
...
pub(crate) fn assert_borrow<Q: Query>() {
    // This looks like an ugly O(n^2) loop, but everything's constant after
    // inlining, so in practice LLVM optimizes it out entirely.
    let mut i = 0;
...
/// A prepared query can be stored independently of the [`World`] to amortize
/// query set-up costs.
...
impl<Q: Query> PreparedQuery<Q> {
    ...
    ///
    /// Avoids the cost of the dynamic borrow checking performed by
    /// [`query`][Self::query].
...
impl<'q, Q: Query> View<'q, Q> {
    ...
    ///
    /// Will yield `None` if the entity does not exist or does not match the
    /// query.
    ///
    /// Does not require exclusive access to the map, but is defined only for
    /// queries yielding only shared references.
    ...
    /// # Safety
    ///
    /// Must not be invoked while any unique borrow of the fetched components of
    /// `entity` is live.
    ...
    /// Like `get_mut`, but allows checked simultaneous access to multiple
    /// entities
    ///
    /// For N > 3, the check for distinct entities will clone the array and take
    /// O(N log N) time.
    ///
    /// # Examples
    ///
