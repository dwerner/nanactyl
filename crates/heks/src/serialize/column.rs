...
//! contiguously. Preferred for data that will be read/written programmatically.
//! Efficient, compact, and highly compressible, but difficult to read or edit
//! by hand.
//!
//! This module builds on the public archetype-related APIs and
//! [`World::spawn_column_batch_at()`], and is somewhat opinionated. For some
//! applications, a custom approach may be preferable.
//!
//! In terms of the serde data model, we treat a [`World`] as a sequence of
//! archetypes, where each archetype is a 4-tuple of an entity count `n`,
//! component count `k`, a `k`-tuple of user-controlled component IDs, and a
//! `k+1`-tuple of `n`-tuples of components, such that the first `n`-tuple
//! contains `Entity` values and the remainder each contain components of the
//! type identified by the corresponding component ID.
...
///
/// `serialize_component_ids` and `serialize_components` must serialize exactly
/// the number of elements indicated by `component_count` or return `Err`.
///
/// Data external to the [`World`] can be exposed during serialization by
/// storing references inside the struct implementing this trait.
///
/// # Example
///
/// ```
...
/// }
/// ```
// Serializing the ID tuple separately from component data allows the
// deserializer to allocate the entire output archetype up front, rather than
// having to allocate storage for each component type after processing the
// previous one and copy into an archetype at the end.
pub trait SerializeContext {
    /// Number of entries that
    /// [`serialize_component_ids`](Self::serialize_component_ids) and
    /// [`serialize_components`](Self::serialize_components) will produce for
    /// `archetype`
    fn component_count(&self, archetype: &Archetype) -> usize;

    /// Serialize the IDs of the components from `archetype` that will be
    /// serialized
    // We use a wrapper here rather than exposing the serde type directly because
    // it's a huge pain to determine how many IDs were written otherwise, and we
    // need that to set the component data tuple length correctly.
    ...
    ///
    /// For each component ID written by `serialize_component_ids`, this method
    /// must write a tuple containing one value for each entity, e.g. using
    /// [`try_serialize`], in the same order. Each tuple's length must
    /// exactly match the number of entities in `archetype`, and there must be
...
/// Serialize all entities in a [`World`] that satisfy the given [`Query`]
/// through a [`SerializeContext`] to a [`Serializer`]
...
pub trait DeserializeContext {
    ...
    ///
    /// Implementers should usually store the deserialized component IDs in
    /// `self` to guide the following `deserialize_components` call.
...
/// Deserializer for a single component type, for use in
...
where
    T: Component + Deserialize<'de>,
{
    /// Construct a deserializer for `entity_count` `T` components, writing into
    /// `batch`
