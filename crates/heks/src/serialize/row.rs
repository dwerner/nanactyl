...
//!
//! Stores each entity's components together. Preferred for data that will be
//! read/written by humans. Less efficient than column-major serialization.
//!
//! This module builds on the public [`World::iter()`] and [`World::spawn_at()`]
//! APIs, and are somewhat opinionated. For some applications, a custom approach
//! may be preferable.
//!
//! In terms of the serde data model, we treat a [`World`] as a map of entity
//! IDs to user-controlled maps of component IDs to data.
...
pub trait SerializeContext {
    ...
    /// Number of entries that [`serialize_entry`](Self::serialize_entity) will
    /// produce for `entity`, if known
    ///
    /// Defaults to `None`. Must be overridden to return `Some` to support
    /// certain serializers, e.g. bincode.
...
// Note: deliberately not implemented in terms of `serialize_satisying::<(), _,
// _>` to avoid an extra loop over the archetypes
...
/// Implements deserialization of entities from a serde [`MapAccess`] into an
/// [`EntityBuilder`]
///
/// Data external to the [`World`] can be populated during deserialization by
/// storing mutable references inside the struct implementing this trait.
///
/// # Example
/// ```
/// # use serde::{Serialize, Deserialize};
