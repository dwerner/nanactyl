...
/// typed components
///
/// Similar to `HashMap<Entity, Vec<Box<dyn Any>>>` where each `Vec` never
/// contains two of the same type, but far more efficient to traverse.
///
/// The components of entities who have the same set of component types are
/// stored in contiguous runs, allowing for extremely fast, cache-friendly
/// iteration.
///
/// There is a maximum number of unique entity IDs, which means that there is a
/// maximum number of live entities. When old entities are despawned, their IDs
/// will be reused on a future entity, and old `Entity` values with that ID will
/// be invalidated.
///
/// ### Collisions
///
/// If an entity is despawned and its `Entity` handle is preserved over the
/// course of billions of following spawns and despawns, that handle may, in
/// rare circumstances, collide with a newly-allocated `Entity` handle. Very
/// long-lived applications should therefore limit the period over which they
/// may retain handles of despawned entities.
pub struct World {
    entities: Entities,
    archetypes: ArchetypeSet,
    /// Maps statically-typed bundle types to archetypes
    bundle_to_archetype: TypeIdMap<u32>,
    /// Maps source archetype and static bundle types to the archetype that an
    /// entity is moved to after inserting the components from that bundle.
    insert_edges: IndexTypeIdMap<InsertTarget>,
    /// Maps source archetype and static bundle types to the archetype that an
    /// entity is moved to after removing the components from that bundle.
...
impl World {
    ...
    ///
    /// Arguments can be tuples, structs annotated with
    /// [`#[derive(Bundle)]`](macro@Bundle), or the result of calling
    /// [`build`](crate::EntityBuilder::build) on an
    /// [`EntityBuilder`](crate::EntityBuilder), which is useful if the set of
    /// components isn't statically known. To spawn an entity with only one
    /// component, use a one-element tuple like `(x,)`.
    ///
    /// Any type that satisfies `Send + Sync + 'static` can be used as a
    /// component.
    ///
    /// # Example
    /// ```
    ...
    ///
    /// Useful for easy handle-preserving deserialization. Be cautious
    /// resurrecting old `Entity` handles in already-populated worlds as it
    ...
    ///
    /// Faster than calling [`spawn`](Self::spawn) repeatedly with the same
    /// components, but requires that component types are known at compile
    /// time.
    ///
    /// # Example
    /// ```
    ...
    ///
    /// The fastest, but most specialized, way to spawn large numbers of
    /// entities. Useful for high performance deserialization. Supports
    ...
    /// Hybrid of [`spawn_column_batch`](Self::spawn_column_batch) and
    ...
    ///
    /// Unlike [`spawn`](Self::spawn), this can be called concurrently with
    /// other operations on the [`World`] such as queries, but does not
    /// immediately create the entities. Reserved entities are not visible
    /// to queries or world iteration, but can be otherwise operated on
    /// freely. Operations that add or remove components or entities, such as
    /// `insert` or `despawn`, will cause all outstanding reserved entities
    /// to become real entities before proceeding. This can also be done
    /// explicitly by calling [`flush`](Self::flush).
    ///
    ...
    /// Ensure at least `additional` entities with exact components `T` can be
    /// spawned without reallocating
    ...
    ///
    /// Preserves allocated storage for reuse but clears metadata so that
    /// [`Entity`] values will repeat (in contrast to
    ...
    ///
    /// Prefer [`query_mut`](Self::query_mut) when concurrent access to the
    /// [`World`] is not required.
    ///
    /// Calling `iter` on the returned value yields `(Entity, Q)` tuples, where
    /// `Q` is some query type. A query type is any type for which an
    /// implementation of [`Query`] exists, e.g. `&T`, `&mut T`, a tuple of
    /// query types, or an `Option` wrapping a query type, where `T` is any
    /// component type. Components queried with `&mut` must only appear once.
    /// Entities which do not have a component type referenced outside of an
    /// `Option` will be skipped.
    ///
    /// Entities are yielded in arbitrary order.
    ///
    /// The returned [`QueryBorrow`] can be further transformed with combinator
    /// methods; see its documentation for details.
    ///
    /// Iterating a query will panic if it would violate an existing unique
    /// reference or construct an invalid unique reference. This occurs when
    /// two simultaneously-active queries could expose the same entity.
    /// Simultaneous queries can access the same component type if and only if
    /// the world contains no entities that have all components required by
    /// both queries, assuming no other component borrows are outstanding.
    ///
    /// Iterating a query yields references with lifetimes bound to the
    /// [`QueryBorrow`] returned here. To ensure those are invalidated, the
    /// return value of this method must be dropped for its dynamic borrows
    /// from the world to be released. Similarly, lifetime rules ensure that
    ...
    ///
    /// Like [`query`](Self::query), but faster because dynamic borrow checks
    /// can be skipped. Note that, unlike [`query`](Self::query), this
    /// returns an `IntoIterator` which can be passed directly to a `for`
    /// loop.
    ...
    ///
    /// Prefer [`query_one_mut`](Self::query_one_mut) when concurrent access to
    /// the [`World`] is not required.
    ///
    /// Call [`get`](QueryOne::get) on the resulting [`QueryOne`] to actually
    /// execute the query. The [`QueryOne`] value is responsible for
    /// releasing the dynamically-checked borrow made by `get`, so it can't
    /// be dropped while references returned by `get` are live.
    ///
    ...
    ///
    /// Like [`query_one`](Self::query_one), but faster because dynamic borrow
    /// checks can be skipped. Note that, unlike
    /// [`query_one`](Self::query_one), on success this returns the
    ...
    /// Short-hand for [`entity`](Self::entity) followed by
    /// [`EntityRef::satisfies`]
    ...
    /// Given an id obtained from [`Entity::id`], reconstruct the still-live
    /// [`Entity`].
    ///
    /// # Safety
    ///
    /// `id` must correspond to a currently live [`Entity`]. A despawned or
    /// never-allocated `id` will produce undefined behavior.
    ...
    ///
    /// Entities are yielded in arbitrary order. Prefer [`query`](Self::query)
    /// for better performance when components will be accessed in
    ...
    /// has. If an entity already has a component of a certain type, it is
    /// dropped and replaced.
    ///
    /// When inserting a single component, see [`insert_one`](Self::insert_one)
    /// for convenience.
    ///
    /// # Example
    /// ```
    ...
    /// The implementation backing [`insert`](Self::insert) exposed so that it
    /// can also be used by [`exchange`](Self::exchange).
    ///
    /// Note that `graph_origin` is always equal to `loc.archetype` during
    /// insertion. Only for exchange, `graph_origin` identifies
    /// the intermediate archetype which would be reached after removal and
    /// before insertion even though the actual component data still resides
    /// in `loc.archetype`.
    fn insert_inner(
        &mut self,
        entity: Entity,
        ...
        unsafe {
            ...
            // Allocate storage in the archetype and update the entity's location to address
            // it
    ...
    ///
    /// Computational cost is proportional to the number of components `entity`
    /// has. The entity itself is not removed, even if no components remain;
    /// use `despawn` for that. If any component in `T` is not present in
    /// `entity`, no components are removed and an error is returned.
    ///
    ...
    pub fn remove<T: Bundle + 'static>(&mut self, entity: Entity) -> Result<T, ComponentError> {
        ...
        if loc.archetype != target {
            // If we actually removed any components, the entity needs to be moved into a
            // new archetype
            let (source_arch, target_arch) = index2(
                &mut self.archetypes.archetypes,
                loc.archetype as usize,
            ...
            if let Some(moved) = unsafe {
                source_arch.move_to(old_index, |src, ty, size| {
                    // Only move the components present in the target archetype, i.e. the
                    // non-removed ones.
    ...
    ///
    /// This has the same effect as calling [`remove::<S>`](Self::remove) and
    /// then [`insert::<T>`](Self::insert), but is more efficient as the
    /// intermediate archetype after removal but before insertion is skipped.
    ...
    /// # Safety
    ///
    /// `entity` must have been previously obtained from this [`World`], and no
    /// unique borrow of the same component of `entity` may be live
    /// simultaneous to the returned reference.
    ...
    /// Convert all reserved entities into empty entities that can be iterated
    /// and accessed
    ///
    /// Invoked implicitly by operations that add or remove components or
    /// entities, i.e. all variations of `spawn`, `despawn`, `insert`, and
    /// `remove`.
    ...
    ///
    /// Useful for dynamically scheduling concurrent queries by checking borrows
    /// in advance, and for efficient serialization.
    ...
    ///
    /// Store the current value after deriving information from
    /// [`archetypes`](Self::archetypes), then check whether the value
    /// returned by this function differs before attempting an
    /// operation that relies on its correctness. Useful for determining whether
    /// e.g. a concurrent query execution plan is still correct.
    ///
    /// The generation may be, but is not necessarily, changed as a result of
    /// adding or removing any entity or component.
    ///
    /// # Example
    /// ```
    /// # use heks::*;
...
/// Types that can be components, implemented automatically for all `Send + Sync
/// + 'static` types
///
...
impl ArchetypeSet {
    fn new() -> Self {
        // `flush` assumes archetype 0 always exists, representing entities with no
        // components.
        Self {
    ...
    fn get_insert_target(&mut self, src: u32, components: &impl DynamicBundle) -> InsertTarget {
        ...
        // ordered, we can identify elements in one but not the other efficiently with
        // parallel iteration.
        let mut src_ty = 0;
