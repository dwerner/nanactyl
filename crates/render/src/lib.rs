...
pub struct VulkanRendererState {
    ...
    pub presenter: Option<Pin<Box<dyn Present + Send + Sync>>>,
}
...
impl VulkanBase {
    ...
    let required_extension_names = ash_window::enumerate_required_extensions(&win_ptr).unwrap().to_vec();
    //TODO: setup VK_LAYER_KHRONOS_validation
    let create_info = vk::InstanceCreateInfo::builder()
    .application_info(application_info)
    .enabled_extension_names(&required_extension_names).build();

    let instance = unsafe { entry.create_instance(&create_info, None) }.unwrap();
    let surface =
        unsafe { ash_window::create_surface(&entry, &instance, &win_ptr, None) }
            .unwrap();
    ...
    Self::record_submit_commandbuffer(
     ...
     let depth_image_view_info = vk::ImageViewCreateInfo::builder()
            ...
            let depth_image_view = unsafe {device
                .create_image_view(&depth_image_view_info, None)
            }.unwrap();

            let semaphore_create_info = vk::SemaphoreCreateInfo::default();

            let present_complete_semaphore = unsafe {device
                .create_semaphore(&semaphore_create_info, None)
            }.unwrap();
            let rendering_complete_semaphore = unsafe{ device
                .create_semaphore(&semaphore_create_info, None)
            }.unwrap();
...
fn record_submit_commandbuffer<F>(
    device: &Device,
